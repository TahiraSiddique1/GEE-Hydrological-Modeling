...
// Load the ERA ImageCollection
var rainfall = ee.ImageCollection("ECMWF/ERA5_LAND/HOURLY");

// Specify the desired date range (replace with your actual dates)
var startDate = ee.Date('2018-01-01');
var endDate = ee.Date('2018-12-31');

// Filter the collection for the specified date range
rainfall = rainfall.filterDate(startDate, endDate);

// Get the minimum and maximum dates within the filtered collection
var range = rainfall.reduceColumns(ee.Reducer.minMax(), ['system:time_start']);
...
// Limit the number of days to 5000 for charting
var diff = ee.Number(diff_).min(ee.Number(500));

// Create a list of dates from the filtered date range
var timestep_mean = ee.List.sequence(0, diff).map(function(n) {
  var start = ee.Date(range.get('min')).advance(n, 'day');
  var end = start.advance(1, 'day');

  return rainfall
      .filterDate(start, end)  // Filter again within the loop for daily averages
      .mean().multiply(24)  // mm/day
      .set('system:time_start', start);
});

// Create an ImageCollection from the daily means
var rainfall = ee.ImageCollection(timestep_mean).select('total_precipitation');
...
var runoff_func = function(image) {
  ...
  var AMC = image.select('total_precipitation')
  var ppt = image.select('total_precipitation')
  ...
  var Q2 = image.expression(
    '((ppt-(0.2*S_image))**2)/(ppt-(0.2*S_image)+S_image)', {
      'ppt': image.select('total_precipitation'),
...
Map.addLayer (RainfallRunoff.select('total_precipitation').sum(), pptVis, 'Rainfall')
